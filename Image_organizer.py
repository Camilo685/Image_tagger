...
def export_images(event, image_list, source = None):
	...
	for image in image_list:
		...
		shutil.copyfile(src + image[0], pathname + image[0])
				...
				with OpenImageDialog("Choose image(s)...", parent = self) as imgdialog:
					...
					if pathname == program_folder:
						...
						for image in selected_lst:						
							image_split = image.split("/") if "/" in image else image.split("\\")
...
class SearchFrame(wx.Frame):    
	def __init__(self, parent = None):
		...
		self.export_img.Bind(wx.EVT_BUTTON, lambda event: export_images(event, [[self.dataset[clk_img][0], True] for clk_img in self.clicked_images] if self.clicked_images else [[img_name[0], True] for img_name in self.append_all(0)[0]]))
