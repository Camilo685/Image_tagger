import wx, os, json, shutil, send2trash, random, uuid, filetype
...
#original_icons_folder = os.path.abspath(".original_icons/") + "/"
...
def check_icons():
	...
	if mssng:
		...
		Repo.clone_from("https://github.com/Camilo685/Image_tagger", repo_dir)
		icon_list = is_image(path_name = repo_dir + "/icons/", check = True)
		for icon_msn in mssng:
			for icon_file in icon_list:
				if icon_msn in icon_file:					
					for d_i_idx, d_i in enumerate(display_icon_list):
						if d_i == icon_msn:
							thumb(icon_file, 30, source = repo_dir + "/icons/", target_folder = icons_folder)
							display_icon_list[d_i_idx] = [icon_msn, wx.Bitmap(wx.Image(icons_folder + icon_file, wx.BITMAP_TYPE_ANY))]
							break
					break
...
def thumb(img_name, width, source = program_folder, new_name = None, target_folder = None, height = -1, mask = False):
	...
	if image_type == "png":
		...
		image = Image.open(source + img_name).convert("RGBA")
	else:
		image = Image.open(source + img_name).convert("RGB")
...
def check_mask(img_list, deleted_imgs = False, images_source = None, width = 100):
	...
	for img in img_list:
		...
		if not os.path.exists(thb_fd + splt[0] + "_" + str(width) + "_msk." + splt[-1]):
			thumb(img, width, new_name = splt[0] + "_" + str(width) + "_msk." + splt[-1], source = src, target_folder = thb_fd, mask = True)
			thumb(img, width, new_name = splt[0] + "_" + str(width) + "." + splt[-1], source = src, target_folder = thb_fd)
...
#		unmarked_checkbox_bitmap = wx.Bitmap(wx.Image(original_icons_folder + '/unmarked_checkbox.png'), wx.BITMAP_TYPE_ANY)
#		marked_checkbox_bitmap = wx.Bitmap(wx.Image(original_icons_folder + '/marked_checkbox.png'), wx.BITMAP_TYPE_ANY)
...
class MainFrame(wx.Frame):    
	...
	def check_img_data_exits(self):
		...
		tmp_list = tmp_list if not added else is_image(check = True)
		
		check_mask(tmp_list)
			
		thb_list = is_image(path_name = thumbs_folder, check = True)
			...
			while until_cancel:
				with OpenDirectoryDialog("Choose a folder...", parent = self) as dirdialog:
					...
					if pathname == program_folder:
						...
						until_cancel = False
			images_selected = is_image(pathname)			
		else:
			while until_cancel:
...
class DisplayFrame(wx.Frame):
	...
	def load_img_data(self):
		...
		if self.img_selected[self.current_image][1]:
			...
			self.img = thumb(self.img_selected[self.current_image][0], 1000, height = 580)
		else:
			...
			self.img = thumb(self.img_selected[self.current_image][0], 1000, source = self.images_source, height = 580)
