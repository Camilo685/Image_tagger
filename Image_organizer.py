import wx, os, json, shutil, send2trash, random, uuid, pathlib, functions
...
temp_thumbs_folder = ""
icons_folder = ""
image_dataset_name = "dataset_test.json"
tag_dataset_name = "tag_dataset.json"

def main():
	global display_icon_list
	global program_folder
	global tmp_folder
	global thumbs_folder
	global temp_thumbs_folder
	global icons_folder
	
	program_data = functions.save_load_file(op_type = "rb")
	for saved_data in program_data:
		display_icon_list = program_data[1]
		program_folder = program_data[2]
		tmp_folder = program_data[3]
		thumbs_folder = program_data[4]
		temp_thumbs_folder = program_data[5]
		icons_folder = program_data[6]
...
def export_images(event, image_list, source = None):
	ok_folder = False
	while not ok_folder:
	...
	for image in image_list:
		src = program_folder if image[1] else source
		shutil.copyfile(src + image[0], pathname)
...
class MainFrame(wx.Frame):    
	def __init__(self):
		...
		try:
			complete_dataset = functions.save_load_file(file_name = image_dataset_name, op_type = "r")
		except:
			functions.save_load_file(file_name = image_dataset_name, to_dump = {}, op_type = "w")
			complete_dataset = functions.save_load_file(file_name = image_dataset_name, op_type = "r")
		try:
			complete_tag_dataset = functions.save_load_file(file_name = tag_dataset_name, op_type = "r")
		except:
			functions.save_load_file(file_name = tag_dataset_name, to_dump = {}, op_type = "w")
			complete_tag_dataset = functions.save_load_file(file_name = tag_dataset_name, op_type = "r")
	...
	def check_img_data_exits(self):
		...
		tmp_list = functions.is_image(path_name = program_folder, check = True)
		deleted = False
		added = False
		...
		if added or deleted:
			functions.save_load_file(file_name = image_dataset_name, to_dump = complete_dataset, op_type = "w")
			functions.save_load_file(file_name = tag_dataset_name, to_dump = complete_tag_dataset, op_type = "w")
		
		tmp_list = tmp_list if not added else functions.is_image(path_name = program_folder, check = True)
			...
			while until_cancel:
				with OpenDirectoryDialog("Choose a folder...", parent = self) as dirdialog:
					...
					if pathname == program_folder:
						...
						until_cancel = False
			images_selected = functions.is_image(path_name = pathname)			
		else:
			while until_cancel:
...
class SearchFrame(wx.Frame):    
	def __init__(self, parent = None):
		...
		self.export_img = wx.Button(self.sf_main_panel, -1, label = "Export images...", name = "export_img")
		self.export_img.Bind(wx.EVT_BUTTON, lambda event: export_images([[self.dataset[clk_img][0], True] for clk_img in self.clicked_images] if self.clicked_images else [[img_name[0], True] for img_name in self.append_all(0)[0]]))
		...
		tag_box_sizer.Add(self.export_img, 0, wx.EXPAND | wx.ALL, 5)
...
#		print("\n")
	...
	def append_all(self, current_image):
	...
	def on_open_edit(self, event):
		...
		if event.GetEventType() == 10035:
			current_image_list, current_image = self.append_all(int(event.GetEventObject().GetName()))
		else:
			###For button events###
			if event.GetEventObject().GetName() == "Edit_image":
				to_edit_bool = True
			if not len(self.clicked_images) > 1:
				current_image_list, current_image = self.append_all(self.clicked_images[0])
			else:
				self.clicked_images.sort()
				current_image_list = [[self.dataset[clk_img][0], True, clk_img] for clk_img in self.clicked_images]
...
class DisplayFrame(wx.Frame):
	def __init__(self, images_selected, to_edit_bool = False, images_source = None, parent = None, current_image = 0, general_tags = None, random_img = False):
		...
		self.export_img.Bind(wx.EVT_BUTTON, lambda event: export_images([self.img_selected, self.images_source]))
		...
		self.tag_box_sizer.Add(self.export_img, 0, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, 10)
	...
	def on_close(self, event):
		if event == None:
			...
			if self.images_source:
				...
				if not self.discard_changes(yes_lb = "Save", no_lb = "Discard"):
					return
	...
	def save_to_dataset(self, event, save_current = True, save_all = False):
		...
		functions.save_load_file(file_name = image_dataset_name, to_dump = complete_dataset, op_type = "w")
		functions.save_load_file(file_name = tag_dataset_name, to_dump = self.current_tag_count, op_type = "w")
